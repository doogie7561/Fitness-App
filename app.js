    ...
    setTimeout(() => {
      ...
      if(!p || !p.onboardingComplete){
        ...
        if(hero && !localStorage.getItem("rpgOnboardingDismissed")) {
          /* v37 disabled auto tab clear */
          /* v37 disabled auto-switch */
        }
      }
    }, 400);
  }
...
/* v37 Assessment Page Placement + Load Fix */
(function(){
  "use strict";
  const $ = id => document.getElementById(id);
  const safeParse = (k,f)=>{try{return JSON.parse(localStorage.getItem(k)||"null")??f}catch(e){return f}};
  ...
  function ensureOneTabVisible(){
    ...
    if(!tabs.some(t => t.classList.contains("active"))){
      const today = document.getElementById("today") || tabs[0];
  ...
  function showAssessmentPage(){
    ...
    if(assessment) assessment.classList.add("active");
  }
  ...
  function showStep(n){
    ...
    steps.forEach(el => el.classList.toggle("active", Number(el.dataset.rpgStep) === state.step));
    const bar = $("rpgProgressBar");
    if(bar) bar.style.width = max ? `${Math.round((state.step/max)*100)}%` : "0%";
  }

  function openWizard(){
    const wizard = $("rpgWizard");
    if(!wizard){
      alert("Profile setup wizard is missing. Re-upload the latest files.");
      return;
    }
  ...
  function initV37(){
    ...
    // Make all tab buttons work even if old init failed.
    document.querySelectorAll("[data-tab]").forEach(btn => {
      btn.addEventListener("click", () => {
        const target = btn.getAttribute("data-tab");
        document.querySelectorAll(".tab").forEach(t => t.classList.remove("active"));
        const section = $(target);
        if(section) section.classList.add("active");
        if(target === "assessment") showProfilePreview();
      });
    });
    ...
    // If no profile exists, highlight Assessment tab but do not break current page.
    if(!safeParse("userProfile", null)){
      const btn = document.querySelector('[data-tab="assessment"]');
      if(btn) btn.classList.add("needs-setup");
    }
  }

  document.addEventListener("DOMContentLoaded", initV37);
})();
