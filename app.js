...
function renderHeader(){let p=profile(),t=targets(); $('title').textContent=p?`${p.name}'s 195 Fitness`:'195 Goal Fitness'; $('phase').innerHTML=p?`Goal: <span data-sensitive>${p.weight} → ${p.goal} lb</span> • Calories <span data-sensitive>${t.calLow}-${t.calHigh}</span> • Protein <span data-sensitive>${t.proteinLow}-${t.proteinHigh}g</span>`:'Complete first-time setup.'}
