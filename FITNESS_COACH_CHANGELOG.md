...
## Version 92 — Recovery Defaults + Avatar Marker Polish
### Bug Fixes
- Fixed new-user recovery defaults.
- New users with no workout history now start at 100% recovery for all body parts.
- Recovery meaning is now explicit:
  - 100% = fully ready
  - 80–100% = ready
  - 40–79% = recovering
  - 0–39% = fatigued
...
### Avatar Polish
- Replaced large percentage badges on the avatar with compact status dots.
- Tapping a marker still shows exact recovery percentage and status.
- Added note for users with no workout history explaining that recovery starts at 100%.

### Portraits
- Replaced the five class portrait files with cleaner fantasy portrait assets while preserving the five-file avatar system.


## Version 93 — Class-Based Workout Demos
### Workout Demo Changes
- Exercise demos now use the user's current class as the demo subject.
- Demo modal shows the class portrait.
- Demo title now says things like `Sage Demo: Goblet Squat`.
- Added class-specific coaching lines.

### Preserved
- Existing Workout layout.
- Exercise cards.
- Guided workout flow.
- Recovery overlay.
- Five-file avatar system.


## Version 95 — Avatar Visual Polish
### Workout Visual Fixes
- Fixed portrait crop by fitting full portrait inside the card.
- Increased portrait card height slightly for better fantasy art display.
- Reduced recovery marker size so markers no longer dominate the artwork.
- Repositioned recovery markers closer to actual body locations.
- Upgraded portrait label from `Level 1 Sage • Gear` to class-title styling.
- Kept the v2 portrait filenames.

### Preserved
- Existing Workout logic.
- Recovery defaults.
- Avatar-guided workout selection.
- Exercise cards.
- Guided workout flow.


## Version 97 — Avatar Layout + V2 Portrait Fix
### Fixes
- Permanently fixed the portrait loader line to use `${classKey}-v2.png`.
- Prevented accidental double `-v2-v2` filenames.
- Restored avatar and status bars to a side-by-side layout.
- Cropped the portrait art higher so built-in card text is no longer the focus.
- Kept app-owned class label and recovery markers on top of the portrait card.


## Version 99 — Implemented V3 Photorealistic Avatar System
- Implemented the approved V3 photorealistic fantasy avatar direction.
- Added male/female versions for every class.
- Active portrait loader now uses `${classKey}-${getAvatarStyle()}-v3.png`.
- Added Avatar Style selector in Settings.
- Status bars remain beside avatar.
- Old v1/v2 portrait files removed from package.

## Version 100 — Set Completion UI
- Logging now tracks individual sets.
- Completed set boxes turn green with a checkmark.
- Added progress dots and set count labels.
- Exercise card turns complete when all sets are logged.


## Version 101 — Runtime Set Completion Fix
### Bug Fix
- Fixed set boxes not changing after tapping Log Set.
- Runtime patch now attaches directly to rendered exercise cards.
